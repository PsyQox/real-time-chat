    ...
    return(
        <div className="mt-10 flex flex-col items-center">
            <div className="w-2/4 h-1/4 items-start justify-center">
                <ul className="text-left h-full max-h-96 overflow-y-auto">
                        {messages?.map((message, i)=>{
                            return(
                                <li className={`${message.from === 'Me' ? 'bg-slate-300 text-right text-black p-3 rounded-lg my-3' : 'bg-blue-800 text-white p-3 rounded-lg my-3'}`} key={i}>{message.from}: {message.body}</li>
                            )
                        })}
            ...
            <div className="mt-3 w-2/4">
                ...
                <input type="text" className="w-11/12 border border-gray-500 rounded-l-lg p-2 h-12" value={myMessage} onChange={(e) => setMyMessage(e.target.value)}
                />
